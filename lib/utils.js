  ...
  return buildRequestBody()
      ...
      .then(result => {
        ...
        const transformedResult =
            cfg.resultTransformExpression && cfg.resultTransformExpression.length > 0 ?
                jsonata(cfg.resultTransformExpression).evaluate(result) :
                result;

        if(cfg.splitResult && Array.isArray(transformedResult)) {
          // Walk through chain of promises: https://stackoverflow.com/questions/30445543/execute-native-js-promise-in-series
          const masterPromise = Promise.resolve();
          transformedResult.reduce(function(p, item) {
            return p.then(function() {
              const output = messages.newMessageWithBody(item);
              output.attachments = msg.attachments;
              return emitter.emit('data', output);
            });
          }, masterPromise);
          return masterPromise;
        } else {
          const output = messages.newMessageWithBody(transformedResult);
          output.attachments = msg.attachments;
          return emitter.emit('data', output);
        }
      })
