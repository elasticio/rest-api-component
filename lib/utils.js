...
module.exports.processMethod = async function (msg, cfg) {
  ...
  return buildRequestBody()
    .then(() => {
      emitter.logger.debug('Got request body');
      return request(requestOptions);
    })
    .then(checkErrors)
    .then(processResponse)
    .then(async (result) => {
      emitter.logger.debug('Got request output');
  ...
  function checkOAuth2Keys(keys) {
    emitter.logger.debug('Check OAuth2 keys');
    if (!keys) {
  ...
  async function fetchNewToken() {
    ...
    emitter.logger.debug('New OAuth2 token fetched');
  ...
  async function getValidToken() {
    ...
    emitter.logger.debug('Emit OAuth2 updateKeys');
    ...
    return Promise.resolve(response.body).then(JSON.parse)
      ...
      .catch((e) => {
        emitter.logger.error(
          'Parsing to JSON object is failed. Return response as is',
          e,
        );
