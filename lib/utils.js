...
module.exports.processMethod = function (isAction, msg, cfg) {
  ...
  return buildRequestBody()
      ...
      .then(result => {
        debug("Component output: %o", JSON.stringify(result));
  ...
  * */
  function checkErrors(response) {
    ...
    debug("Response statusCode %o", statusCode);
  ...
  * */
  function processResponse(response) {
    debug('HTTP Response headers: %o', JSON.stringify(response.headers));
    debug('HTTP Response body: %o', response.body);
    ...
    if (contType) {
      if (contType.includes('json')) {
        ...
        if (contType.includes('image') || contType.includes('msword') || contType.includes('msexcel') || contType.includes('pdf') || contType.includes('csv')) {
          attachment.addAttachment(msg, uuidv1 + '_' + new Date().getTime(), response.body, response.headers['content-length'], contType);
          debug('added attachments %j', msg.attachments);
        }
        return Promise.resolve({body: response.body.blob()});
      }
    }
