...
module.exports.processMethod = async function (isAction, msg, cfg) {
  ...
  switch (auth.type) {
    ...
    case authTypes.OAUTH2:
      headers.push({
        key: 'Authorization',
        value: `"Bearer ${await getValidToken()}"`,
      });
      break;
    default:
      if (existingAuthHeader) {
  ...
  async function getValidToken() {
    const oauth2 = cfg.oauth2.keys;
    if (!oauth2) {
      throw new Error('cfg.oauth2.keys can not be empty');
    }
    const tokenExpiryTime = new Date(oauth2.tokenExpiryTime);
    const now = new Date();
    ...
    cfg.oauth2.keys = await fetchNewToken();
    cfg.oauth2.keys.tokenExpiryTime = (new Date(tokenRefreshStartTime.getTime()
        + (cfg.oauth2.keys.expires_in * 1000))).toISOString();
