/* eslint-disable max-len,no-shadow,no-param-reassign,no-underscore-dangle,no-use-before-define,consistent-return,no-trailing-spaces,space-infix-ops */
...
module.exports.processMethod = async function (isAction, msg, cfg) {
  ...
  async function fetchNewToken() {
    ...
    // add warning for absent refresh
    if (!oauth2.keys.refresh_token) {
      emitter.logger.warn('No refresh token detected. System may not behave properly');
    }
  ...
  * */
  function checkErrors(response) {
    ...
    if (statusCode >= 300 && statusCode < 400) {
      ...
      const { oauth2 } = auth;
      if ((auth.type===authTypes.OAUTH2)&&!oauth2.keys.refresh_token) { /* Alert User of Platform Limitation possibily causing error */
        errString += ' Note: No refresh_token was detected. This might have caused this error. Please refer to https://github.com/elasticio/rest-api-component#user-content-known-limitations';
      }
