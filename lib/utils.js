...
const { getRateLimitDelay, rateLimit } = require('./helpers');

const HTTP_ERROR_CODE_REBOUND = new Set([408, 423, 429, 500, 502, 503, 504]);

const REQUEST_TIMEOUT = process.env.REQUEST_TIMEOUT ? parseInt(process.env.REQUEST_TIMEOUT, 10) : 100000; // 100s
...
module.exports.processMethod = async function (msg, cfg) {
  ...
  const { auth } = cfg;
  let requestTimeout = cfg.requestTimeoutPeriod ? parseInt(cfg.requestTimeoutPeriod, 10) : REQUEST_TIMEOUT;

  if (!(requestTimeout >= 0)) {
    emitter.logger.warn('Incorrect Request Timeout input found - setting to 100 sec by default');
