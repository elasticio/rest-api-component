...
module.exports.processMethod = async function (msg, cfg, isAction = false) {
  const emitter = this;
  ...
  const url = isAction ? JsonataTransform.jsonataTransform(msg, { expression: config.url }, emitter) : config.url;
  ...
  switch (auth.type) {
    ...
    case authTypes.OAUTH2:
      emitter.logger.debug('auth = %j', auth);
  ...
  function checkOAuth2Keys(keys) {
    emitter.logger.debug('Check keys = %j', keys);
    if (!keys) {
  ...
  async function fetchNewToken() {
    ...
    emitter.logger.debug('New token fetched : %j', authTokenResponse);
  ...
  async function getValidToken() {
    ...
    if (!auth.oauth2.keys.refresh_token) {
      emitter.logger.debug('refresh_token was not returned, use old refresh_token');
    ...
    emitter.logger.debug('Emit updateKeys = %j', auth);
