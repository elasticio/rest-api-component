...
module.exports.processMethod = function (isAction, msg, cfg) {
    ...
    return new Promise((resolve, reject) => {
        request[formattedMethod](requestOptions, function (error, response, body) {
            ...
            try {
                console.log(`Have got response ${response.statusCode} headers ${JSON.stringify(response.headers)} and response "${body}"`);
                if (response.statusCode >= 400 && cfg.throwErrors) {
                    throw new Error(`Unexpected response code, expecting between 200 and 299 but `+
                        `have got ${response.statusCode} instead. URI: ${requestOptions.uri}, ` +
                        `headers ${JSON.stringify(response.headers)}.` +
                        `If you don't want to have errors reported in the flow switch the config "Throw Error on HTTP Failures" off`);
                }
                ...
                try {
                    ...
                    if (response.headers['content-type']) {
                        const cType = contentType.parse(response);
                        if (cType.type.includes('xml')) {
                            console.log(`Detected content type "${cType.type}" seems to be XML, will try to parse it`);
                            parseString(body, {
                                trim: false,
                                normalize: false,
                                explicitArray: false,
                            ...
                            }, function(parsingError, jsonResult) {
                                if (parsingError) {
                                    reject(parsingError);
                                }
                                return resolve(messages.newMessageWithBody(jsonResult));
                            });
                        }
                    } else {
                        throw err;
