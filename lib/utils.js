...
const { checkRateLimitParams, rateLimit } = require('./helpers');
...
module.exports.processMethod = async function (msg, cfg) {
  ...
  checkRateLimitParams(cfg, emitter.logger);
  ...
  return buildRequestBody()
    ...
    .then(async (result) => {
      ...
      await rateLimit(cfg, emitter.logger);
  ...
  */
  ...
  // async function rateLimit(cfg) {
  //   const { delayBetweenCalls } = cfg;
  //   if (delayBetweenCalls) {
  //     const delay = parseInt(delayBetweenCalls, 10);
  //     emitter.logger.info('Delay Between Calls is set to:', delay);
  //     emitter.logger.debug('Delay is start', new Date());
  //     await sleep(delay);
  //     emitter.logger.debug('Delay is done', new Date());
  //   }
  // }
};
