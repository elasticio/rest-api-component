...
module.exports.processMethod = function (isAction, msg, cfg) {
      ...
      if (!followRedirect) {
        ...
        if (cfg.dontThrowErrorFlg) {
          return Promise.resolve({
            ...
            statusMessage: `${response.statusMessage || 'Redirection error.'} Please check "Follow redirect mode" if You want to use redirection in your request. `,
  ...
  function processResponse(response) {
    ...
    if (!response.body) {
      return Promise.resolve(buildSimpleResponseStructure({}));
    }
    ...
    if (contType) {
      if (contType.includes('json')) {
        return Promise.resolve(buildSimpleResponseStructure(response).then(JSON.parse);
        ...
        return xml2js(response.body, parseOptions).then(buildSimpleResponseStructure);
      }
    } else {
      debug("unknown content-type received. trying to parse like JSON");
      return Promise.resolve(response.body).then(JSON.parse)
        .then(buildSimpleResponseStructure)
        .catch(e => {
          debug(`parsing to JSON object is failed. Error: ${e}. Returning response as is`);
          return buildFullResponseStructure(body);
        });
    }

    function buildFullResponseStructure(body) {
      return {
    ...
    function buildSimpleResponseStructure(body) {
      return body;
    }
  }
}